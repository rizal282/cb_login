...
class _PenerimaanState extends State<Penerimaan> {
  ...
  void getPenerimaan() async {
    final response = await http.get("http://192.168.43.171/cb-login/user/penerimaan");
  ...
  void kirimBarang(String kodeBarang, String qty) async {
    var response = await http.post("http://192.168.43.171/cb-login/user/cekKodeBarang", body: {
    ...
    if(listkode.length == 1){
        await http.post("http://192.168.43.171/cb-login/user/updateStokBarang", body: {
        "kodeBarang" : kodeBarang,
        "stokbaru" : qty,
      });
  ...
  void hapusPenerimaan(String id) async {
    await http.post("http://192.168.43.171/cb-login/user/hapusPenerimaan", body: {
      "id" : id
    });
