...
class _PermintaanState extends State<Permintaan> {
  ...
  void getPermintaan() async {
    final response = await http.get("http://192.168.43.171/cb-login/user/permintaan");
  ...
  void _hapusPermintaan(String id) async {
    await http.post("http://192.168.43.171/cb-login/user/hapusPermintaan", body: {
      "id" : id
    });
  ...
  void _cetakPdf(context) async {
    var data = await http.get("http://192.168.43.171/cb-login/user/permintaan"); 
